...
void Solenoid::set(unsigned long onDuration, unsigned long offDuration, byte cycles, unsigned long startDelay) {
  ...
  Serial << F("Solenoid setting.  pin=") << this->pin << F(". on/off=") << this->onDuration << "/";
  Serial << this->offDuration << F(" ms. cycles=") << this->cyclesTotal << F(". start delay=") << this->startDelay << endl; 
}
